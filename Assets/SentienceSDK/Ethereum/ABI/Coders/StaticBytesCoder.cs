...
namespace Sentience.ABI
{
    public class StaticBytesCoder : ICoder
    {
        ...
        public object Decode(byte[] encoded)
        {
            try
            {
                string encodedStr = SentienceCoder.ByteArrayToHexString(encoded);
                return SentienceCoder.HexStringToByteArray(DecodeFromString(encodedStr));
            }
            catch (Exception ex)
            {
        ...
        public byte[] Encode(object value)
        {
            try
            {
                ...
                ///enc(X) = enc(k) pad_right(X), i.e.the number of bytes is encoded as a uint256 followed by the actual value of X as a byte Sentience, followed by the minimum number of zero-bytes such that len(enc(X)) is a multiple of 32.
                string encodedString = EncodeToString(value);
                byte[] encoded = SentienceCoder.HexStringToByteArray(encodedString);
                return encoded;
            }
            catch (Exception ex)
            {
        ...
        public string EncodeToString(object value)
        {
            try
            {
                string valueStr = SentienceCoder.ByteArrayToHexString(((FixedByte)value).Data);
    ...
    public static class StaticBytesCoderExtensions
    {
        ...
        {
            return SentienceCoder.HexStringToByteArray(_coder.DecodeFromString(encoded.Replace("0x", "")));
        }
    }
}
