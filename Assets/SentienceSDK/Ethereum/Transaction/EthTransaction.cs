...
namespace Sentience.Transactions
{
    public class EthTransaction
    {
        ...
        public string RLPEncode()
        {
            ...
            txToEncode.Add(SentienceCoder.HexStringToByteArray(To));
            txToEncode.Add(Value.ToByteArray(true, true));
            txToEncode.Add(SentienceCoder.HexStringToByteArray(Data));
            if (V!=null && R!= null && S!= null)
            {
                txToEncode.Add(SentienceCoder.HexStringToByteArray(V));
                txToEncode.Add(SentienceCoder.HexStringToByteArray(R));
                txToEncode.Add(SentienceCoder.HexStringToByteArray(S));
            }else
            {
                txToEncode.Add(SentienceCoder.HexStringToByteArray(ChainId));
                txToEncode.Add(BigInteger.Zero.ToByteArray(true, true));
                txToEncode.Add(BigInteger.Zero.ToByteArray(true, true));
            }

            byte[] encodedList = RLP.RLP.Encode(txToEncode);
            return SentienceCoder.ByteArrayToHexString(encodedList).EnsureHexPrefix();
        }
        ...
        public string SignAndEncodeTransaction(EthWallet wallet)
        {
            ...
            string signingHash = SentienceCoder.KeccakHash(encoded_signing).EnsureHexPrefix();
            (string v, string r, string s) = wallet.SignTransaction(SentienceCoder.HexStringToByteArray(signingHash), ChainId);
            this.V = v;
            this.R = r;
            this.S = s;
