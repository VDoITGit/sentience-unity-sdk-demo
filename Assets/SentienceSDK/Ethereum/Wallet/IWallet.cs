...
namespace Sentience.Wallet
{
    public interface IWallet 
    {
        ...
        {
            string hashed = SentienceCoder.ByteArrayToHexString(SentienceCoder.KeccakHash(pubkey));
            int length = hashed.Length;
            string address = hashed.Substring(length - 40);

            address = SentienceCoder.AddressChecksum(address);
            return address;
        }
        ...
        public static byte[] PrefixedMessage(byte[] message)
        {
            ...
            byte[] message191 = ByteArrayExtensions.ConcatenateByteArrays(SentienceCoder.HexStringToByteArray("19"), Encoding.UTF8.GetBytes("Ethereum Signed Message:\n"));
            byte[] messageLen = Encoding.UTF8.GetBytes((message.Length).ToString());
            if (!message.Take(message191.Length).SentienceEqual(message191))
            {
