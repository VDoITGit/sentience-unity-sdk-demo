...
namespace Sentience.Wallet
{
    public class EthWallet : IWallet
    {
        ...
        {
            privKey = ECPrivKey.Create(SentienceCoder.HexStringToByteArray(_privateKey));
            pubKey = privKey.CreatePubKey();
        }
        ...
        public async Task<string> SignMessage(byte[] message, byte[] chainId = null)
        {
            ...
            byte[] message32 = SentienceCoder.KeccakHash(PrefixedMessage(message));
        ...
        public string SignMessageWithPrivateKey(string privateKey, string message)
        {
            ...
            message32 = SentienceCoder.KeccakHash(PrefixedMessage(Encoding.UTF8.GetBytes(message)));

            ECPrivKey privKey = Context.Instance.CreateECPrivKey(SentienceCoder.HexStringToByteArray(privateKey));
            return EthSignature.Sign(message32, privKey);
        }
        ...
        public string SignByteArray(string privateKey, byte[] byteArray)
        {
            ...
            prefixed = SentienceCoder.KeccakHash(PrefixedMessage(byteArray));
            ECPrivKey privKey = Context.Instance.CreateECPrivKey(SentienceCoder.HexStringToByteArray(privateKey));
        ...
        public async Task<bool> IsValidSignature(string signature, string message, Chain chain = Chain.None)
        {
            ...
            byte[] hashedMessage = SentienceCoder.KeccakHash(messagePrefix);
