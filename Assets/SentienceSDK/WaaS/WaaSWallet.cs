...
namespace Sentience.WaaS
{
    public class WaaSWallet : IWallet
    {
        ...
        private const string _SentienceCreatedContractEvent = "CreatedContract(address)";
        private string _builderApiKey;

        public WaaSWallet(Address address, string sessionId, IIntentSender intentSender)
        {
            _address = address;
            _httpClient = new HttpClient("https://api.Sentience.app/rpc");
            _intentSender = intentSender;
            SessionId = sessionId;
            _builderApiKey = SentienceConfig.GetConfig().BuilderAPIKey;
        }
        ...
        public async Task<ContractDeploymentReturn> DeployContract(Chain network, string bytecode, string value = "0")
        {
            ...
            {
                string topic = SentienceCoder.KeccakHashASCII(_SentienceCreatedContractEvent);
