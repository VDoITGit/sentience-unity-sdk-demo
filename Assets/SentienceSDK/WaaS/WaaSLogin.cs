...
namespace Sentience.WaaS
{
    public class WaaSLogin : ILogin 
    {
        ...
        public WaaSLogin(IValidator validator = null)
        {
            ...
            {
                throw SentienceConfig.MissingConfigError("WaaS Version");
            }
            _waasVersion = waasVersion;

            ConfigJwt configJwt = SentienceConfig.GetConfigJwt();

            string rpcUrl = configJwt.rpcServer;
            if (string.IsNullOrWhiteSpace(rpcUrl))
            {
                throw SentienceConfig.MissingConfigError("RPC Server");
            }
            ...
            {
                throw SentienceConfig.MissingConfigError("Project ID");
            }
            ...
            _authenticator = new OpenIdAuthenticator(SentienceCoder.KeccakHashASCII(_sessionId).EnsureHexPrefix());
            ...
            {
                Debug.LogWarning("AWS config not found in config key. Email sign in will not work. Please contact Sentience support for more information.");
            }
        }
        ...
        public async Task ConnectToWaaS(string idToken, LoginMethod method)
        {
            ...
            try
            {
                ...
                string email = Sentience.Authentication.JwtHelper.GetIdTokenJwtPayload(idToken).email;
