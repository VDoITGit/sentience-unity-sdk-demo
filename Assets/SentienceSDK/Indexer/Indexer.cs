...
namespace Sentience
{
        ...
        = new Dictionary<BigInteger, string>
    {
        ...
        private static string _builderApiKey = SentienceConfig.GetConfig().BuilderAPIKey;
        ...
        /// <returns></returns>
        /// <exception>Throws if the chainID isn't a Sentience-supported chain.</exception>
        private static string HostName(BigInteger chainID)
        {
            var indexerName = IndexerNames[chainID];
            return $"https://{indexerName}-indexer.Sentience.app";
        }
        ...
        private static async Task<string> HttpPost(BigInteger chainID, string endPoint, object args, int retries = 0)
        {
            ...
            catch (FileLoadException e)
            {
                if (e.Message.Contains($"{(int)HttpStatusCode.TooManyRequests}"))
                {
                    if (retries == 5)
                    {
                        Debug.LogError("Sentience server rate limit exceeded, giving up after 5 retries..." + "\nCurl-equivalent request: " + curlRequest);
                    }
                    else
                    {
                        
                        Debug.LogWarning($"Sentience server rate limit exceeded, trying again... Retries so far: {retries}" + "\nCurl-equivalent request: " + curlRequest);
