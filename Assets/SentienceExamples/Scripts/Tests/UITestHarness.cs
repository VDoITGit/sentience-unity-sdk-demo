...
namespace SentienceExamples.Scripts.Tests
{
    public class UITestHarness : MonoBehaviour
    {
        ...
        {
            SentienceSampleUI.IsTesting = true;
            SceneManager.LoadScene("SentienceExamples/Scenes/Tests");
            while (_ui == null)
            {
                yield return null; // Allow object to load
                _ui = FindObjectOfType<SentienceSampleUI>();
        ...
        {
            SentienceSampleUI.InitialPanel = initialPanel;
            SentienceSampleUI.InitialPanelOpenArgs = args;
            SentienceSampleUI.IsTesting = false; // Allows test to run
            _ui.Start();
